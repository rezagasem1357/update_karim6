...
class _DeliveryScreenState extends State<DeliveryScreen> {
  ...
  void _openShareReportChooser() {
    ...
    if (!hasSales && !hasManifests) {
      _showSuccessMessage(
          '⚠️ هنوز گزارش فروش یا بارنامه‌ای برای اشتراک وجود ندارد');
      return;
    }
...
class SalesProfitScreen extends StatefulWidget {
  ...
  const SalesProfitScreen({super.key, required this.products});
...
class _SalesProfitScreenState extends State<SalesProfitScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('📈 محاسبه سود فروش'),
        backgroundColor: Colors.green.shade700,
        foregroundColor: Colors.white,
      ),
      body: Column(
        children: [
          ...
          Padding(
            ...
            child: Align(
              alignment: Alignment.centerRight,
              child: Text(
                'تعداد کالاها: ${_toPersianDigits(products.length.toString())}',
                style: TextStyle(
                  color: Theme.of(context).colorScheme.onSurfaceVariant,
                  fontSize: 13,
                ),
